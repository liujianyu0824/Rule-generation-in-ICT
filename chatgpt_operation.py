...
openai.api_key = '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'

logging.basicConfig(
    filename='C:\\Users\\86183\\Desktop\\huawei_project\\logs\\chatgpt_operation.log',
    format='%(asctime)s - %(levelname)s - %(name)s - %(message)s',
    datefmt='%m/%d/%Y %H:%M:%S',
    level=logging.INFO)
logger = logging.getLogger(__name__)

data_file = 'C:\\Users\\86183\\Desktop\\huawei_project\\data\\operation_test.json'

with open(data_file, 'r', encoding='utf-8') as f:
    original_data = json.load(f)

# 定义需要过滤的前缀和后缀停用词
pre_stop_words = ["如果", "导致", "将", "会", "需要", "可以", "当", "总是", "若", "是", "导致","将会", "才"]
post_stop_words = ["时", "后"]
...
#         j = 0
#         for pos in factor_pos:
#             if pos[0] != data["rule_list"][j]['span'][0] or pos[1] != data["rule_list"][j]['span'][1]:
#                 error_instance.append(data)
#                 break
#             j = j + 1
        print(correct_num,correct_subset,correct_superset)
#
# with open("error_instance2.json", "w", encoding="utf-8") as f:
#     json.dump(error_instance, f, ensure_ascii=False, indent=2)
